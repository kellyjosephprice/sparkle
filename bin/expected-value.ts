...
import { GameState, Die } from "../src/types";
...
function getScoringDice(state: GameState, strategy: StrategyConfig): Die[] {
    ...
    if (strategy.scoringStrategy === 'minimal') {
        const bestGroup = result.groups.reduce((prev, curr) => curr.dice.length < prev.dice.length ? curr : prev);
...
function simulateTurn(state: GameState, strategy: StrategyConfig): GameState {
    ...
    while (currentState.extraDicePool > 0 && currentState.dice.length < 6) {
        currentState = gameEngine.processCommand(currentState, { type: "ADD_EXTRA_DIE" }).state;
    }
    
    currentState = gameEngine.processCommand(currentState, { type: "ROLL_DICE" }).state;
    ...
    while (!currentState.gameOver) {
        ...
        if (currentState.lastRollSparkled) {
            ...
            const hasBanked = currentState.dice.some(d => d.banked);

            // Strategy: prefer discard over reroll?
            if (strategy.preferDiscardOverReroll && activeCount <= 2 && hasBanked) {
                currentState = gameEngine.processCommand(currentState, { type: "DISCARD_UNSCORED" }).state;
                continue;
            }
            
            if (currentState.rerollsAvailable > 0) {
                currentState = gameEngine.processCommand(currentState, { type: "RE_ROLL" }).state;
                continue;
            } else if (hasBanked) {
                currentState = gameEngine.processCommand(currentState, { type: "DISCARD_UNSCORED" }).state;
                continue;
            } else {
                return gameEngine.processCommand(currentState, { type: "END_TURN", isSparkled: true }).state;
            }
        }
        ...
        for (const d of toSelect) {
            currentState = gameEngine.processCommand(currentState, { type: "TOGGLE_DIE", dieId: d.id }).state;
        }
        
        const staged = getStagedScore(currentState);
        const potential = currentState.totalScore + currentState.bankedScore + staged;
        const remaining = getActiveDice(currentState).filter(d => !d.staged).length;
        
        const meetsThreshold = potential >= currentState.threshold;
        let riskThreshold = 2;
        if (strategy.reluctance > 0) {
            riskThreshold += Math.floor((currentState.bankedScore / 2000) * 10 * strategy.reluctance);
            if (riskThreshold > 5) riskThreshold = 5;
        }
        
        if (meetsThreshold) {
            if (remaining === 0 || (remaining > riskThreshold)) {
                currentState = gameEngine.processCommand(currentState, { type: "BANK_DICE" }).state;
                while (currentState.extraDicePool > 0 && currentState.dice.length < 6) {
                    currentState = gameEngine.processCommand(currentState, { type: "ADD_EXTRA_DIE" }).state;
                }
                currentState = gameEngine.processCommand(currentState, { type: "ROLL_DICE" }).state;
                ...
                currentState = gameEngine.processCommand(currentState, { type: "BANK_DICE" }).state;
                return gameEngine.processCommand(currentState, { type: "END_TURN" }).state;
            }
        } else {
            currentState = gameEngine.processCommand(currentState, { type: "BANK_DICE" }).state;
            while (currentState.extraDicePool > 0 && currentState.dice.length < 6) {
    ...
    while (!state.gameOver && state.turnNumber <= MAX_TURNS) {
        ...
        if (state.turnNumber > turnBefore && !state.gameOver) {
            if (state.upgradeOptions.length > 0) {
                state = gameEngine.processCommand(state, { type: "SELECT_UPGRADE", upgradeType: state.upgradeOptions[0].type }).state;
            }
        }
        history.push(state);
    }
...
const strategies: Record<string, StrategyConfig> = {
  "Risky": { preferDiscardOverReroll: false, scoringStrategy: 'all', reluctance: 0 },
  "Safe Minimal": { preferDiscardOverReroll: true, scoringStrategy: 'minimal', reluctance: 0.8 },
  "Balanced": { preferDiscardOverReroll: true, scoringStrategy: 'all', reluctance: 0.3 }
};
...
for (const [name, strategy] of Object.entries(strategies)) {
    ...
    const results = Array.from({ length: MAX_TURNS + 2 }, () => ({ total: 0, count: 0, gain: 0 }));
    
    for (let i = 0; i < SIMULATIONS; i++) {
        const history = runGame(strategy);
        for (let t = 1; t <= MAX_TURNS; t++) {
            const endOfTurn = history.find(s => s.turnNumber === t + 1 || (s.gameOver && s.turnNumber <= t + 1));
            if (endOfTurn) {
                if (endOfTurn.turnNumber > t) {
                    results[t].count++;
                    results[t].total += endOfTurn.totalScore;
                    const startOfTurn = history.find(s => s.turnNumber === t);
                    if (startOfTurn) results[t].gain += (endOfTurn.totalScore - startOfTurn.totalScore);
                } else break;
            } else break;
        }
    }
