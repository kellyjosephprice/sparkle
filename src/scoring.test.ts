...
describe("calculateScore", () => {
  describe("Empty and non-scoring dice", () => {
    ...
    it("should return 0 for non-scoring dice", () => {
      expect(calculateScore(makeDice([2, 3, 4, 6]))).toBe(0);
    });

    it("should return 0 for single non-scoring die", () => {
      expect(calculateScore(makeDice([2]))).toBe(0);
      expect(calculateScore(makeDice([3]))).toBe(0);
      expect(calculateScore(makeDice([4]))).toBe(0);
      expect(calculateScore(makeDice([6]))).toBe(0);
    });
  });

  describe("Single 1s and 5s", () => {
    it("should score single 1 as 100 points", () => {
      expect(calculateScore(makeDice([1]))).toBe(100);
    });

    it("should score single 5 as 50 points", () => {
      expect(calculateScore(makeDice([5]))).toBe(50);
    });

    it("should score two 1s as 200 points", () => {
      expect(calculateScore(makeDice([1, 1]))).toBe(200);
    });

    it("should score two 5s as 100 points", () => {
      expect(calculateScore(makeDice([5, 5]))).toBe(100);
    });

    it("should score one 1 and one 5 as 150 points", () => {
      expect(calculateScore(makeDice([1, 5]))).toBe(150);
    });

    it("should ignore non-scoring dice when scoring 1s and 5s", () => {
      expect(calculateScore(makeDice([1, 2, 3, 5]))).toBe(150);
    });
  });

  describe("Three of a kind", () => {
    it("should score three 1s as 1000 points", () => {
      expect(calculateScore(makeDice([1, 1, 1]))).toBe(1000);
    });

    it("should score three 2s as 200 points", () => {
      expect(calculateScore(makeDice([2, 2, 2]))).toBe(200);
    });

    it("should score three 3s as 300 points", () => {
      expect(calculateScore(makeDice([3, 3, 3]))).toBe(300);
    });

    it("should score three 4s as 400 points", () => {
      expect(calculateScore(makeDice([4, 4, 4]))).toBe(400);
    });

    it("should score three 5s as 500 points", () => {
      expect(calculateScore(makeDice([5, 5, 5]))).toBe(500);
    });

    it("should score three 6s as 600 points", () => {
      expect(calculateScore(makeDice([6, 6, 6]))).toBe(600);
    });
  });

  describe("Four of a kind", () => {
    it("should score four 1s as 2000 points (1000 * 2)", () => {
      expect(calculateScore(makeDice([1, 1, 1, 1]))).toBe(2000);
    });

    it("should score four 2s as 400 points (200 * 2)", () => {
      expect(calculateScore(makeDice([2, 2, 2, 2]))).toBe(400);
    });

    it("should score four 5s as 1000 points (500 * 2)", () => {
      expect(calculateScore(makeDice([5, 5, 5, 5]))).toBe(1000);
    });

    it("should score four 6s as 1200 points (600 * 2)", () => {
      expect(calculateScore(makeDice([6, 6, 6, 6]))).toBe(1200);
    });
  });

  describe("Five of a kind", () => {
    it("should score five 1s as 4000 points (1000 * 4)", () => {
      expect(calculateScore(makeDice([1, 1, 1, 1, 1]))).toBe(4000);
    });

    it("should score five 2s as 800 points (200 * 4)", () => {
      expect(calculateScore(makeDice([2, 2, 2, 2, 2]))).toBe(800);
    });

    it("should score five 5s as 2000 points (500 * 4)", () => {
      expect(calculateScore(makeDice([5, 5, 5, 5, 5]))).toBe(2000);
    });

    it("should score five 6s as 2400 points (600 * 4)", () => {
      expect(calculateScore(makeDice([6, 6, 6, 6, 6]))).toBe(2400);
    });
  });

  describe("Six of a kind", () => {
    it("should score six 1s as 8000 points (1000 * 8)", () => {
      expect(calculateScore(makeDice([1, 1, 1, 1, 1, 1]))).toBe(8000);
    });

    it("should score six 2s as 1600 points (200 * 8)", () => {
      expect(calculateScore(makeDice([2, 2, 2, 2, 2, 2]))).toBe(1600);
    });

    it("should score six 5s as 4000 points (500 * 8)", () => {
      expect(calculateScore(makeDice([5, 5, 5, 5, 5, 5]))).toBe(4000);
    });

    it("should score six 6s as 4800 points (600 * 8)", () => {
      expect(calculateScore(makeDice([6, 6, 6, 6, 6, 6]))).toBe(4800);
    });
  });

  describe("Straight (1-2-3-4-5-6)", () => {
    it("should score straight as 1500 points", () => {
      expect(calculateScore(makeDice([1, 2, 3, 4, 5, 6]))).toBe(1500);
    });

    it("should score straight regardless of order", () => {
      expect(calculateScore(makeDice([6, 4, 2, 5, 1, 3]))).toBe(1500);
      expect(calculateScore(makeDice([3, 1, 4, 5, 6, 2]))).toBe(1500);
    });
  });

  describe("Three pairs", () => {
    it("should score three pairs as 1500 points", () => {
      expect(calculateScore(makeDice([1, 1, 2, 2, 3, 3]))).toBe(1500);
    });

    it("should score three pairs regardless of values", () => {
      expect(calculateScore(makeDice([4, 4, 5, 5, 6, 6]))).toBe(1500);
      expect(calculateScore(makeDice([1, 1, 5, 5, 6, 6]))).toBe(1500);
    });

    it("should score three pairs regardless of order", () => {
      expect(calculateScore(makeDice([2, 3, 2, 4, 3, 4]))).toBe(1500);
    });
  });
  ...
  describe("Mixed combinations", () => {
    it("should score three 1s plus extra 1s correctly", () => {
      ...
      expect(calculateScore(makeDice([1, 1, 1, 1, 2, 3]))).toBe(2000);
    });

    it("should score three of a kind plus single scoring dice", () => {
      // Three 2s = 200, one 1 = 100, one 5 = 50
      expect(calculateScore(makeDice([2, 2, 2, 1, 5, 6]))).toBe(350);
    });
    ...
    it("should score multiple sets of three of a kind", () => {
      ...
      expect(calculateScore(makeDice([2, 2, 2, 3, 3, 3]))).toBe(500);
    });

    it("should score three of a kind with extra single scoring dice", () => {
      // Three 6s = 600, one 1 = 100
      expect(calculateScore(makeDice([6, 6, 6, 1]))).toBe(700);
    });
  });

  describe("Edge cases", () => {
    it("should not score two pairs as three pairs", () => {
      expect(calculateScore(makeDice([1, 1, 2, 2]))).toBe(200); // Just two 1s
    });

    it("should not score incomplete straight", () => {
      // Missing the 6, so just score the 1 and 5
      expect(calculateScore(makeDice([1, 2, 3, 4, 5]))).toBe(150);
    });
    ...
    it("should prioritize straight over individual scoring", () => {
      ...
      expect(calculateScore(makeDice([1, 2, 3, 4, 5, 6]))).toBe(2500);
    });
  });
});
